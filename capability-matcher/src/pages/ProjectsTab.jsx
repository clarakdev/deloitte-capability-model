...
// Shows the project creator metadata inline so the dashboard presents a
// clearer ownership summary for the user's personal project list.
  ...
  return (
    <div className="card" style={{ marginBottom: 0 }}>
      <div className="card-head">
        ...
        <span className="badge badge-green">{projects.length} projects</span>
      </div>

      {projects.length === 0 ? (
        <p style={{ color: '#aaaaaa', fontSize: 13 }}>No projects are available for this account.</p>
      ) : (
        <div style={{ display: 'grid', gap: 12 }}>
          {projects.map((project) => (
            <div key={project.id} style={{ border: '1px solid #2a2a2a', borderRadius: 8, padding: 14 }}>
              ...
              <p style={{ color: '#aaaaaa', fontSize: 13, marginBottom: 8 }}>{project.client || 'Client not specified'}</p>
              <p style={{ color: '#aaaaaa', fontSize: 13, marginBottom: 8 }}>
